...
function kirimWA() { const nama = escapeHTML(document.getElementById("nama").value.trim()); const alamat = escapeHTML(document.getElementById("alamat").value.trim()); const nohp = escapeHTML(document.getElementById("nohp").value.trim()); if(!/^[0-9]{10,13}$/.test(nohp)) { alert("No HP harus 10-13 digit angka ya!"); return; } if(nama == "" ||
