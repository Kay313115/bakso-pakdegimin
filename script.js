...
document.addEventListener('DOMContentLoaded', function() { document.querySelectorAll('.bintang').forEach(bintang => { bintang.addEventListener('click', function(){ ratingTerpilih = parseInt(this.dataset.nilai); updateBintang(); }) tampilkanUlasan(); })
...
function hitungOngkir(alamat) { alamat = alamat.toLowerCase(); if(alamat.includes("blok aj")) return 3000; else if(alamat.includes("vgh")) return 5000; else if(alamat.includes("babelan")) return 7000; else if(alamat.includes("bekasi")) return 10000; else return 15000; }
