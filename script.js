...
function hitungOngkir(alamat){
  ...
  else if(alamat.includes("villa gading")||alamat.includes("vgh")||alamat.includes("gading harapan")){return 5000;}
  else if(alamat.includes("babelan")){return 7000;}
  else if(alamat.includes("bekasi")){return 10000;}
  else{return 15000;}
}
...
async function kirimUlasan(){
  ...
  try{
    await fetch(URL_API,{method:"POST",body:JSON.stringify({nama,pesan,rating:ratingTerpilih})});
    ...
    tampilkanUlasan();
  }catch(e){alert('Gagal kirim. Cek: 1. URL bener 2. Apps Script udah Deploy "Anyone"');}
}
...
async function tampilkanUlasan(){
  ...
  try{
    ...
    if(ulasan.length===0){list.innerHTML='<p style="text-align:center;opacity:0.7;">Belum ada ulasan. Jadilah yang pertama!</p>';return;}
    list.innerHTML='';
  ...
  }catch(e){list.innerHTML='<p style="text-align:center;color:red;">Gagal memuat ulasan</p>';}
}
